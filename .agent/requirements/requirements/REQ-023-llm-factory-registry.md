...
versions:
  ...
  - version: 2
    ...
    author: ai
    context: "需求审查发现提供商计数错误"
    reason: "修正提供商常量定义计数为 17 个（实际列出的名称为 17 个）"
    snapshot: "LLM 工厂注册，提供商常量 17 个"
---
...
## 验收标准
- [x] 提供商常量定义（17个）：`ProviderOpenAI`/`ChatGPT`/`Anthropic`/`Gemini`/`DeepSeek`/`GLM`/`Qwen`/`MiniMax`/`Kimi`/`Moonshot`/`Zhipu`/`Baidu`/`Doubao`/`DashScope`/`ModelScope`/`LinkAI`/`Xunfei`
