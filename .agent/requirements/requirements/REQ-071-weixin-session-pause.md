...
## 验收标准
- [x] 会话暂停机制：errcode -14 时暂停所有 API 请求，而非立即 re-login — `pkg/channel/weixin/session_guard.go`
- [x] 暂停时长：可配置，默认 60 分钟 — `pkg/channel/weixin/session_guard.go:16-17` + `Config.SessionPauseMinutes`
- [x] 暂停期间跳过所有 getUpdates/sendMessage 调用 — `handleSessionExpired()` + `Send()` 中 AssertActive 检查
- [x] 暂停到期后自动恢复轮询，无需重新 QR 登录 — `IsPaused()` 自动过期清理
- [x] 连续失败退避：3 次连续暂停后触发 re-login — `ShouldRelogin()` + `maxSessionPauseAttempts=3`
- [x] 区分暂时性过期和永久失效：暂停恢复后仍失败才触发 re-login — `handleSessionExpired()` 逻辑
